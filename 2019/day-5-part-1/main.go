 ...
 * if the parameter is 50, its value is the value stored at address 50 in memory. Until now, all parameters have
 * been in position mode.
 *
 ...
 * value in an instruction. Parameter modes are single digits, one per parameter, read right-to-left from the opcode:
 * the first parameter's mode is in the hundreds digit, the second parameter's mode is in the thousands digit, the
 ...
 * of this operation, 33 * 3 = 99, is written according to the third parameter, 4 in position mode, which also works like it
 ...
 * modes, function correctly. For each test, it will run an output instruction indicating how far the result of the test was
 * from the expected value, where 0 means the test was successful. Non-zero outputs mean that a function is not working correctly;
 ...
 * immediately by a halt means the program finished. If all outputs were zero except the diagnostic code, the diagnostic program
 * ran successfully.
 *
...
func (intCode *IntCode) PrintDebug(additionalInfo string) {
	...
	log.Printf(
		...
		intCode.inputBuffer,
		intCode.code[intCode.instructionPointer:intCode.instructionPointer+5],
	)
}
